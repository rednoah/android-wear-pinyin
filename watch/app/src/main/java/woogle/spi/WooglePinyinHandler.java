...
//			this.isKeyPressedConsumed = c!=-1 ? true : false;
		...
		case KeyEvent.KEY_TYPED:
			ke = (KeyEvent)event;
			if (this.isKeyPressedConsumed 
					&& w.getLocale().equals(Locale.SIMPLIFIED_CHINESE)) {
				ke.consume();
			}
		}
	}
			...
			for (int i=0; i < driver.length; i++) {
				try {
					Runtime.getRuntime().exec(
						driver[i] + ":/Program Files/Internet Explorer/IEXPLORE.EXE " + url);
					break;
				} catch (Exception e) {
					w.log(e.getMessage());
				}
		...
		if (w.getLocale().equals(Locale.SIMPLIFIED_CHINESE)
			...
			if (1 <= digit && digit <= 5) {
				...
				if (this.display.isFinished()) {
					...
					this.chart.CKYParseWithSelect(
							WoogleInputMethod.Lm, WoogleInputMethod.Dm, WoogleInputMethod.Dic,
							c.c.row, c.c.column);
					this.setCand();
				}
			}
	...
	public void backspaceAction(KeyEvent e) {
		if (w.getLocale().equals(Locale.SIMPLIFIED_CHINESE) 
			...
			if (!display.isEmpty()) {
				...
				this.chart.CKYParse(this.display.getPinyin(), 
						WoogleInputMethod.Lm, WoogleInputMethod.Dm, WoogleInputMethod.Dic);
				this.setCand();
			}
			this.isKeyPressedConsumed = true;
